...
if(isset($_POST['add_baru'])){
	...
	if($_POST['list']=="gejala"){
		...
		$hasil=_simpan_penyakit($nama_penyakit);
		if($hasil==0){
			$_SESSION ['add']='<div class="alert alert-success"> Data berhasil ditambah </div>';
		}else{
			 $_SESSION ['add']='<div class="alert alert-error"> Data gagal ditambah </div>';
		...
		$hasil=_simpan_rule($keterangan,$jika_ya,$jika_tidak,$mulai,$selesai,$nilai_cf);
		if($hasil==0){
			$_SESSION ['add']='<div class="alert alert-success"> Data berhasil ditambah </div>';
		}else{
			 $_SESSION ['add']='<div class="alert alert-error"> Data gagal ditambah </div>';
...
?>
			<p><strong style="font-size: 20px"> Tambah Data Penyakit </strong>
			<hr>
			</p>
			<br>
			<form class="form-horizontal" id="frm_penyakit" name="frm_penyakit" action="form.php?list=<?php echo $_GET['list'];?>" method="post">
            	<input type="hidden" name="list" value="<?php echo $_GET['list'];?>" />
                 	<div class="control-group">
        			<label class="control-label"> Nama penyakit </label>
           				<div class="controls"> 
                        <Textarea name="nama_penyakit" cols="5" style="width:300px;"></Textarea> 
                        </div>
                     </div>
 					<div class="control-group">  
                    	<div class="controls">              
...
?>
			<p><strong style="font-size: 20px"> Tambah Rule Diagnosa </strong>
			<hr>
			</p>
			<br>

...
if(isset($_POST['update'])){
	...
	if($_POST['list']=="gejala"){
	...
	}elseif($_POST['list']=="penyakit"){
		$id_penyakit=$_GET['id'];
		$nama_penyakit=$_POST['nama_penyakit'];
		
		$hasil= _update_penyakit($id_penyakit,$nama_penyakit);
		if($hasil==0){
			$_SESSION ['add']='<div class="alert alert-success"> Data berhasil diubah </div>';	
		}else{
			 $_SESSION ['add']='<div class="alert alert-error"> Data gagal diubah </div>';
		...
		//if($respon==0){
			...
			$sql_update="update tb_gejala set jika_ya='".$jika_ya."',jika_tidak='".$jika_tidak."',mulai='".$mulai."',selesai='".$selesai."',nilai_cf='".$nilai_cf."' where id_gejala='".$id_gejala."'";
		...
		//}
			...
			$_SESSION ['add']='<div class="alert alert-success"> Nilai CF tidak boleh lebih dari 1</div>';	
		}
		//simpan aturan baru
		//$sql_aturan="";
		...
		for($xi=0;$xi<$jmlAr;$xi++){
			...
			$sql_aturan="insert into tb_aturan(id_aturan,id_gejala,id_penyakit,nilai_cf)values('".$id_aturan."','".$_POST['chk'][$xi]."','".$id_penyakit."','".$_POST['cf'][$xi]."')";
			
			echo $xi."|".$_POST['chk'][$xi]."|".$_POST['cf'][$xi]."</br>";
			mysql_query($sql_aturan)or die(mysql_error());
			
			//if(!empty
			$sql_n_cf="update into tb_aturan set nilai_cf='".$_POST['cf'][$xi];*/
		//}
			//$_SESSION ['add']='<div class="alert alert-success"> Data berhasil disimpan </div>';
...
<p><strong style="font-size: 20px"> Aturan Diagnosa </strong>
            ...
            <select id='combo_ya' name='combo_ya' style='width:300px;' required="true" >
                ...
                <select id='combo_tidak' name='combo_selesai' style='width:300px;' required="true" >
                 ...
                 <div class="control-group">
                <div class="controls">
                <label><b>Nilai CF :</b></label>
                <input type="text" style="text-align:right;" name="nilai_cf" value='<?php echo $data['nilai_cf']?>'/>
                </div>
            	</div>
                <div class="control-group">
                <div class="controls">
...
<?php
			...
			$id=$_GET['id'];
			$sql2 = "SELECT * FROM tb_penyakit WHERE id_penyakit='".$id."'";
			$result2 = mysql_query($sql2);
			$data =  mysql_fetch_assoc($result2);
?>
		<p><strong style="font-size: 20px"> Ubah Penyakit </strong>
			<hr>
			</p>
			<br>
